...
top_20 <- str_split(old_man, pattern= " ") %>%
  unlist() %>%
  table() %>%
...
# Following code
# -selects the essays
# -creates a new variable "all_essays" consisting of all 10 essay responses
#  pasted together
# -selects only sex and the all_essays variable
essays <-
  select(profiles, sex, starts_with("essay")) %>%
  mutate(
    all_essays = paste(essay0, essay1, essay2, essay3, essay4, essay5, essay6, essay7, essay8, essay9, sep=" ")
    ) %>%
  select(sex, all_essays) %>%
  tbl_df()
...
essays <- essays %>%
  mutate(
    all_essays = str_replace_all(all_essays, "\n", " "),
    all_essays = str_replace_all(all_essays, "<br />", " ")
  )

# SOLUTION2:
male_words <- subset(essays, sex == "m") %>%
  .[["all_essays"]] %>%
  str_split(" ") %>%
  unlist() %>%
  table() %>%
...
female_words <- subset(essays, sex == "f") %>%
  .[["all_essays"]] %>%
  str_split(" ") %>%
  unlist() %>%
  table() %>%
